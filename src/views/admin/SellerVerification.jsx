...
// Import BASE_URL here. Adjust the path '../../' based on your folder structure!
...
function SellerVerification() {
  ...
  const fetchSellers = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/api/admin/sellers/pending`, {
  ...
  const handleVerify = async (id, status) => {
    ...
    try {
      ...
      // Replaced localhost with BASE_URL
      await axios.post(
        `${BASE_URL}/api/admin/sellers/verify`,
      ...
      <div className="card shadow-sm border-0 rounded-4 overflow-hidden">
        <div className="card-header bg-white py-3 border-bottom">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <span className="badge bg-warning text-dark rounded-pill px-3">{sellers.length} Pending</span>
          </div>
        </div>
