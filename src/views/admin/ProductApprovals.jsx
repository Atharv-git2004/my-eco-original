...
// Import BASE_URL here. Make sure the path matches your folder structure!
...
function ProductApprovals() {
  ...
  const fetchPendingProducts = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/api/admin/products/pending`, {
  ...
  const handleApproval = async (id, status) => {
    ...
    try {
      ...
      // Replaced localhost with BASE_URL
      await axios.post(
        `${BASE_URL}/api/admin/products/verify`,
  ...
  return (
    <div className="product-approvals-wrapper p-4">
      ...
      <div className="d-flex align-items-center justify-content-between mb-4">
        ...
        <span className="badge bg-warning text-dark px-3 py-2 rounded-pill">{products.length} Pending</span>
      </div>
                  ...
                  {products.length === 0 ? (
                    ...
                    products.map((product) => (
                      <tr key={product._id}>
                        ...
                        <td>
                          <div className="text-primary fw-semibold">{product.sellerId?.storeName || "Unknown Seller"}</div>
