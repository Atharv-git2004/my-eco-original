  ...
  const fetchProducts = async () => {
    try {
      ...
      if (res?.status === 200 || res?.status === 201) {
        // Safely extract the products array depending on your API's exact response structure
        const products = res.data?.allProducts || res.data?.products || (Array.isArray(res.data) ? res.data : []);
        setAllProducts(products);
      }
    } catch (err) {
      console.error("Error fetching products:", err);
      setAllProducts([]); // Ensure it remains an array even on error
    } finally {
      setLoading(false);
    }
  };

  // Added Array.isArray check and optional chaining (?.) for extra safety
      ...
      <div className="bg-white border-bottom py-4 mb-4">
        <div className="container text-start">
          ...
          <p className="text-muted mb-0">
            {loading
              ? "Discovering eco-friendly items..."
              : `Showing ${filteredProducts.length} verified sustainable products`}
          </p>
        </div>
      </div>
            ...
            {loading ? (
              ...
              <div className="row g-4 text-start">
                {filteredProducts.map((product) => (
                  <div key={product._id || Math.random()} className="col-md-6 col-lg-4">
