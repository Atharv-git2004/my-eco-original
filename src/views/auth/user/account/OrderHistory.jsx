  ...
  useEffect(() => {
    const fetchMyOrders = async () => {
      try {
        ...
        // Replaced hardcoded localhost with imported BASE_URL
              ...
              <tbody>
                {orders.map((order) => (
                  <tr key={order._id}>
                    {/* Order ID */}
                    <td className="ps-4">
                      <span className="fw-bold text-dark">#{order._id.slice(-8).toUpperCase()}</span>
                    </td>
                    ...
                    {/* Total Amount */}
                    <td className="fw-bold text-success">₹{Number(order.totalPrice).toLocaleString("en-IN")}</td>
